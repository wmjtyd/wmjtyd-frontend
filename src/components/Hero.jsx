...
export function Hero() {
  ...
  const isZh=query["en-US"]==""?false:true;
  return (
      ...
      <h1 className="max-w-4xl mx-auto text-5xl font-medium tracking-tight font-display text-slate-900 sm:text-7xl">
       {isZh?"利用":"Use"}{' '}
        <span className="relative text-blue-600 whitespace-nowrap">
          ...
          <span className="relative">{isZh?"新技术、大数据":"  Big data"}</span>
        </span>{' '}
      </h1>
      <p className="max-w-2xl mx-auto mt-6 text-lg tracking-tight text-slate-700">
      {isZh?"为企业及个人提供方法论， 寻找新的增长点!":"Provide methods for enterprises and individuals to find new growth points!"}
      </p>
      <div className="flex justify-center mt-10 gap-x-6">
        <Button >{isZh?"成功案例":"Successful Cases"}</Button>
        <Button
          href=""
          variant="outline"
        >
          ...
          <span className="ml-3">{isZh?"视频介绍":"Video"}</span>
        </Button>
      </div>
